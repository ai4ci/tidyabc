...
# Source: https://github.com/terminological/ggrrr/blob/HEAD/R/standalone-ggplot-utils.R
# Generated by: pkgtools::use_standalone("terminological/ggrrr", "ggplot-utils")
# ----------------------------------------------------------------------
#
# ---
...
..flt_aes = function(geom, mapping) {
  mapping[names(mapping) %in% geom$aesthetics()]
}
...
.gg_layer = function(
  ...
  return(
    ggplot2::layer(
      ...
      data = data,
      mapping = ..flt_aes(geom, .gg_check_in_data(data, mapping)),
...
.gg_check_for_aes = function(...) {
  dots = rlang::list2(...)
  if (length(dots) > 0 && any(sapply(dots, inherits, "uneval"))) {
    stop(
...
#' @param allow_override are the original mappings kept and `name=value` pairs
#'   act as default and may be overridden by user supplied aesthetics
#'
...
.gg_merge_aes = function(..., allow_override = TRUE) {
  ...
  aess = dots[is_aes]
  others = dots[!is_aes]
  others = others[names(others) != ""]
  out = ggplot2::aes(!!!others)
  for (a in aess) {
    a = eval(a, env)
    if (allow_override) {
      out = out[!names(out) %in% names(a)]
    } else {
      a = a[!names(a) %in% names(out)]
    }
