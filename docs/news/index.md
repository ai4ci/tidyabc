...
- ABC
  - auto calibrate kernel epsilon (? and sample size) on ESS in adaptive
    fits
- Migrate
...
- statistic functions wrappers `dist_fns` S3 class
  ...
  - empirical, transformed and mixture distributions in unified
    framework
  - distribution plotting
- Empirical distributions
  - from weighted data or from cumulative probabilities
  - link function support for constraints on support
  - monotonic spline based fits in Q-Q (cdf) or logit-Z (data) space
    with support for `q`, `p`, `d`, `r` functions.
